...
The extractor SHALL return chapters once each in presentation order with zero-based `index`, one-based inclusive `startSlide` and `endSlide`, and an ordered `slideNumbers` list matching that range.
