...
- [x] 3.2 Configure npm Trusted Publishing for owner `xdoo`, repository `slidev-addon-chapter`, and workflow `publish.yml`, with no long-lived npm token.
- [x] 3.3 Confirm GitHub Actions is permitted to request the required OIDC identity token and that the intended repository/default branch and workflow path match the npm configuration.
...
- [x] 5.2 Create Git tag `v0.1.1` on that commit and create a GitHub Release for the exact tag.
- [x] 5.3 Publish the GitHub Release only after all local gates and npm Trusted Publisher checks pass; inspect the workflow run and npm provenance after publication.
